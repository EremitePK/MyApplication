...
class FragmentMovieDetails : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        initViews(view)
        val movieViewModel =
            ViewModelProvider(requireActivity(), MovieDetailsViewModelFactory()).get(
                MovieDetailsViewModel::class.java
            )
        movieViewModel.loadingCurrentMovie.observe(
            this.viewLifecycleOwner,
            this::loadingMovieDetails
        )
        movieViewModel.currentMovie.observe(this.viewLifecycleOwner, this::updateMovieDetails)
        movieViewModel.currentActorsList.observe(this.viewLifecycleOwner, this::updateAdapterActors)
        actorsAdaptor = ActorsAdapter()
        actorListRecycler?.adapter = actorsAdaptor
        idMovie?.let { movieViewModel.findMovie(it) }
    }
