...
import ru.eremite.myapplication.presentation.movie_details.ActorsAdapter
import ru.eremite.myapplication.presentation.movies_list.*
...
class MainFindViewHolder(itemView: View) : RecyclerViewHolder(itemView) {
    ...
    fun onBind(listViewModel: MoviesListViewModel) {
        nameMovieTextView.text = listViewModel.currentFindName.value
    ...
    fun onBind(
        ...
        when (typeList) {
            0 -> {
                ...
                if (listViewModel.currentMovePosition.value != RecyclerView.NO_POSITION && (listViewModel.currentMovePosition.value
                        ?: 0) < countItem
                ) {
                    listRecyclerView.scrollToPosition(listViewModel.currentMovePosition.value ?: 0)
                } else {
