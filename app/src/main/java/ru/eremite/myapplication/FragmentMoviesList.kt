...
class FragmentMoviesList : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        initViews(view)
        val listViewModel = ViewModelProvider(requireActivity(), MoviesListViewModelFactory(this.requireContext())).get(
    ...
    private fun updateAdapter(listMovies: List<PresentationModelData.MoviePresentation>) {
        ...
        if (moviesAdapter == null) {
            val listViewModel =
                ViewModelProvider(requireActivity(), MoviesListViewModelFactory(this.requireContext())).get(
                    MoviesListViewModel::class.java
                )
